...
const AddUsage = () => {
  ...
  //recent update to show current tracker status on add usage page
  const [tracking, setTracking] = useState(false);
  useEffect(() => {
  const loadTrackerState = async () => {
    try {
      const res = await API.get("/tracker/status");
      setTracking(res.data.enabled);
    } catch (error) {
      console.log("Tracker status load failed");
    }
  };

  loadTrackerState();
}, []);
//toggleTracking recent added
const toggleTracking = async () => {
  try {
    ...
    // tracker app launch
    // if (newState) {
    //   await API.post("/tracker/launch");
    // }

    // enable / disable tracking
    await API.post("/tracker/toggle", {
      enabled: newState
    });
    ...
    console.log("Tracker toggle failed", error);
  }
};
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMsg("✅ Manual usage added successfully!");
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-950 flex items-center justify-center p-6">
      ...
      >
        ...
        {/* Auto added toggle button to start and stop tracker from add usage page */}
